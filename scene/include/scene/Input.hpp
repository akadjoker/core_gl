...
// Polled input state (raylib-style): the app feeds SDL events into the
// On*() callbacks and calls Update() once per frame; everything else reads
// back through Is*/Get*. Behavior nodes (see Behavior.hpp and its
// subclasses) poll this directly instead of taking input as a parameter,
// so a camera/character controller is just `add_child(new FreeFlyBehavior())`
// with no wiring back to the app's event loop.
// Ported from tmp/core/include/Input.hpp + src/Input.cpp.
