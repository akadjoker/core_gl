...
bool SceneRenderer::init()
{
    ...
    // lens flare atlas — embedded PNG (flares_png.h generated by xxd -i)
    {
        scene::Pixmap pm;
        if (pm.load_from_memory(kFlarePng, kFlarePngLen))
        {
            m_flareTex.Load2D(pm.pixels, (int)pm.width, (int)pm.height, gl::TextureFormat::RGBA8);
            m_flareTex.SetFilter(gl::TextureFilter::LINEAR, gl::TextureFilter::LINEAR);
            m_flareTex.SetWrap(gl::TextureWrap::CLAMP_TO_EDGE, gl::TextureWrap::CLAMP_TO_EDGE);
        }
        else
        {
            m_flareTex.Load2D(nullptr, 1, 1, gl::TextureFormat::RGBA8);
        }
    }
    m_lensflare.init_default_flares();
...
// blend, depth off. The LensFlare object bakes NDC geometry; here we
// handle the actual GL draw (shader bind, texture bind, state).
void SceneRenderer::draw_lensflare(Camera3D& cam)
{
    u32 count = m_lensflare.build(cam);
    if (count == 0) return;

    gl::Shader* sh = m_lensflare.shader();
    if (!sh) return;
...
void SceneRenderer::render(Scene& scene, int viewport_w, int viewport_h)
{
    ...
    // lens flare: screen-space, additive, on top of everything
    m_lensflare.set_sun_direction(-m_lightDir); // sun opposite to light dir
    m_lensflare.set_sun_color(Vec3(1.0f, 0.95f, 0.85f));
    draw_lensflare(*camera);
