...
static inline gl::u8 mix_byte(gl::u8 dst, gl::u8 src, float alpha)
{
    ...
    return clamp_to_byte(static_cast<int>(std::lround(
        static_cast<float>(src) * ca + static_cast<float>(dst) * ia)));
}
...
static Color blend_src_over(const Color &src, const Color &dst, float opacity)
{
    ...
    const gl::u32 srcA = clamp_to_byte(static_cast<int>(std::lround(
        (static_cast<float>(src.a()) / 255.0f) * co * 255.0f)));
    ...
    return Color(
        static_cast<gl::u8>((dstR * dstA + srcR * srcA) / outA),
...
static bool assign_loaded_pixels(Pixmap &pm, const unsigned char *src,
                                  int w, int h, int src_comp)
{
    ...
    if (!np) { log_err("malloc failed", ""); return false; }
    memset(np, 0, total);
    ...
    else
    {
        for (int y = 0; y < h; ++y)
            for (int x = 0; x < w; ++x)
            {
                const size_t si = (static_cast<size_t>(y) * static_cast<size_t>(w) + static_cast<size_t>(x)) * static_cast<size_t>(src_comp);
                const size_t di = (static_cast<size_t>(y) * static_cast<size_t>(w) + static_cast<size_t>(x)) * static_cast<size_t>(nc);
                ...
                const unsigned char a = (src_comp == 2) ? src[si + 1]
                                         : (src_comp == 4) ? src[si + 3] : 255;
                if (nc == 3) { np[di+0] = r; np[di+1] = g; np[di+2] = b; }
                else         { np[di+0] = r; np[di+1] = g; np[di+2] = b; np[di+3] = a; }
            }
    }

    if (pm.pixels) free(pm.pixels);
    pm.pixels = np; pm.width = w; pm.height = h; pm.components = nc;
    return true;
}

static bool build_save_buf(const Pixmap &pm, std::vector<unsigned char> &scr,
                            int &sc, const unsigned char *&sp)
{
    if (!pm.pixels || pm.width <= 0 || pm.height <= 0) return false;
    if (pm.components == 1 || pm.components == 3 || pm.components == 4)
    {
        sc = pm.components; sp = pm.pixels; return true;
    }
    sc = 4;
    ...
    for (int y = 0; y < pm.height; y++)
        for (int x = 0; x < pm.width; x++)
        {
            ...
            scr[di+0] = c.r(); scr[di+1] = c.g(); scr[di+2] = c.b(); scr[di+3] = c.a();
        }
    sp = scr.data();
    return true;
}
...
static bool save_tga(const Pixmap &pm, const char *fn)
{
    ...
    if (!f) { log_err("TGA open failed", fn); return false; }
    ...
    {
        if (gray)
            for (int x = 0; x < pm.width; x++) row[x] = pm.get_pixel_color(x, y).r();
        else if (bpp == 24)
            for (int x = 0; x < pm.width; x++)
            {
                const Color c = pm.get_pixel_color(x, y);
                const size_t o = static_cast<size_t>(x) * 3;
                row[o+0] = c.b(); row[o+1] = c.g(); row[o+2] = c.r();
            }
        else
            for (int x = 0; x < pm.width; x++)
            {
                ...
                row[o+0] = c.b(); row[o+1] = c.g(); row[o+2] = c.r(); row[o+3] = c.a();
            }
...
Pixmap::~Pixmap() { if (pixels) free(pixels); }
...
Pixmap::Pixmap(const Pixmap &img, const IntRect &crop)
{
    ...
    pixels = static_cast<unsigned char *>(malloc(
        static_cast<size_t>(width) * static_cast<size_t>(height) * 4));
    memset(pixels, 0, static_cast<size_t>(width) * static_cast<size_t>(height) * 4);
    for (int y = static_cast<int>(crop.y), off = 0;
         y < static_cast<int>(crop.y + crop.height); y++)
    {
        memcpy(pixels + off,
               img.pixels + (y * img.width + static_cast<int>(crop.x)) * components,
               static_cast<size_t>(static_cast<int>(crop.width)) * static_cast<size_t>(components));
        off += static_cast<int>(crop.width) * components;
    }
}

Pixmap::Pixmap(int w, int h, int comp)
    : components(comp), width(w), height(h)
{
    pixels = static_cast<unsigned char *>(malloc(
...
Pixmap::Pixmap(int w, int h, int comp, unsigned char *data)
    : components(comp), width(w), height(h)
{
    pixels = static_cast<unsigned char *>(malloc(
        static_cast<size_t>(w) * static_cast<size_t>(h) * 4));
    memset(pixels, 0, static_cast<size_t>(w) * static_cast<size_t>(h) * 4);
    memcpy(pixels, data, static_cast<size_t>(w) * static_cast<size_t>(h) * static_cast<size_t>(comp));
}
...
{
    set_pixel(x, y,
              static_cast<gl::u8>(rgba),
              static_cast<gl::u8>(rgba >> 8),
              static_cast<gl::u8>(rgba >> 16),
              static_cast<gl::u8>(rgba >> 24));
}
...
gl::u32 Pixmap::get_pixel(gl::u32 x, gl::u32 y) const
{
    ...
    if (components == 2)      return pixels[(y*width+x)*2] | (static_cast<gl::u32>(pixels[(y*width+x)*2+1]) << 8);
    if (components == 3)      return pixels[(y*width+x)*3] | (static_cast<gl::u32>(pixels[(y*width+x)*3+1]) << 8)
                                                         | (static_cast<gl::u32>(pixels[(y*width+x)*3+2]) << 16);
    return pixels[(y*width+x)*4] | (static_cast<gl::u32>(pixels[(y*width+x)*4+1]) << 8)
...
Color Pixmap::get_pixel_color(gl::u32 x, gl::u32 y) const
{
    ...
    {
        r = pixels[(y*width+x)*3]; g = pixels[(y*width+x)*3+1]; b = pixels[(y*width+x)*3+2];
    }
    else
    {
        r = pixels[(y*width+x)*4]; g = pixels[(y*width+x)*4+1];
        b = pixels[(y*width+x)*4+2]; a = pixels[(y*width+x)*4+3];
    }
...
{
    if (pixels)
        memset(pixels, 0, static_cast<size_t>(width) * static_cast<size_t>(height)
...
{
    int w = 0, h = 0, sc = 0;
    unsigned char *ld = stbi_load(fn, &w, &h, &sc, 0);
    if (!ld) { log_err("Failed to load image", fn); return false; }
...
{
    if (!buf || sz == 0) { log_err("Invalid buffer", ""); return false; }
    int w = 0, h = 0, sc = 0;
    unsigned char *ld = stbi_load_from_memory(buf, static_cast<int>(sz), &w, &h, &sc, 0);
    if (!ld) { log_err("Failed to load from memory", stbi_failure_reason()); return false; }
    bool ok = assign_loaded_pixels(*this, ld, w, h, sc);
    stbi_image_free(ld);
    return ok;
}
...
{
    if (!pixels) { log_err("Null pixels, cannot save", fn); return false; }
    if (has_extension(fn, ".tga")) return save_tga(*this, fn);

    std::vector<unsigned char> scr;
    int sc = 0; const unsigned char *sp = nullptr;
    if (!build_save_buf(*this, scr, sc, sp))
    { log_err("Build save buffer failed", fn); return false; }
    ...
    else log_err("Unsupported format", fn);
    return ok;
}
...
void Pixmap::draw_line(int x1, int y1, int x2, int y2, const Color &color)
{
    ...
    while (true)
    {
        set_pixel(static_cast<gl::u32>(x1), static_cast<gl::u32>(y1),
        ...
        for (int dy = 0; dy < h; dy++)
            for (int dx = 0; dx < w; dx++)
            {
                ...
                if (px >= 0 && px < width && py >= 0 && py < height)
                    set_pixel(static_cast<gl::u32>(px), static_cast<gl::u32>(py),
                              color.r(), color.g(), color.b(), color.a());
            }
    }
    else
...
void Pixmap::draw_circle(int cx, int cy, int radius, const Color &color, bool filled)
{
    ...
    else
    {
        ...
        while (x <= y)
        {
            ...
            pt(cx+x,cy+y); pt(cx-x,cy+y); pt(cx+x,cy-y); pt(cx-x,cy-y);
            pt(cx+y,cy+x); pt(cx-y,cy+x); pt(cx+y,cy-x); pt(cx-y,cy-x);
            if (d < 0) d += 4*x + 6;
            else { d += 4*(x-y) + 10; y--; }
            x++;
        }
...
void Pixmap::blend_pixel(gl::u32 x, gl::u32 y, const Color &color, float opacity, BlendMode mode)
{
    ...
    case BlendMode::add:
        out = Color(
            clamp_to_byte(dst.r() + static_cast<int>(std::lround(static_cast<float>(color.r()) * co))),
            clamp_to_byte(dst.g() + static_cast<int>(std::lround(static_cast<float>(color.g()) * co))),
            clamp_to_byte(dst.b() + static_cast<int>(std::lround(static_cast<float>(color.b()) * co))),
            clamp_to_byte(dst.a() + static_cast<int>(std::lround(static_cast<float>(color.a()) * co))));
        break;
    case BlendMode::multiply:
        out = Color(
            mix_byte(dst.r(), clamp_to_byte(static_cast<int>((static_cast<float>(dst.r()) * static_cast<float>(color.r())) / 255.0f)), co),
            mix_byte(dst.g(), clamp_to_byte(static_cast<int>((static_cast<float>(dst.g()) * static_cast<float>(color.g())) / 255.0f)), co),
            mix_byte(dst.b(), clamp_to_byte(static_cast<int>((static_cast<float>(dst.b()) * static_cast<float>(color.b())) / 255.0f)), co),
...
void Pixmap::draw_pixmap(const Pixmap &src, int x, int y)
{
    ...
    for (int sy = 0; sy < src.height; sy++)
        for (int sx = 0; sx < src.width; sx++)
        {
            ...
            {
                if (c.a() == 255)
                    set_pixel(static_cast<gl::u32>(dx),static_cast<gl::u32>(dy),c.r(),c.g(),c.b(),c.a());
                else if (c.a() > 0)
                {
                ...
                set_pixel(static_cast<gl::u32>(dx),static_cast<gl::u32>(dy),c.r(),c.g(),c.b(),255);
        }
}
...
void Pixmap::draw_pixmap_blended(const Pixmap &src, int x, int y, const IntRect &sr,
    ...
    for (int y = 0; y < sh; y++)
        for (int x = 0; x < sw; x++)
        {
            ...
            if (tx >= 0 && tx < width && ty >= 0 && ty < height)
            {
                ...
                set_pixel(static_cast<gl::u32>(tx), static_cast<gl::u32>(ty),
                          c.r(), c.g(), c.b(), c.a());
            }
        }
}
...
Pixmap* Pixmap::apply_gaussian_blur(int /*radius*/) const
{
    ...
    Pixmap *r = new Pixmap(width, height, components);
    float k[3][3] = {{1/16.0f,2/16.0f,1/16.0f},{2/16.0f,4/16.0f,2/16.0f},{1/16.0f,2/16.0f,1/16.0f}};
    for (int y = 0; y < height; y++)
        for (int x = 0; x < width; x++)
        {
            ...
            for (int ky=-1;ky<=1;ky++)
                ...
                {
                    int px=std::max(0,std::min(width-1,x+kx)), py=std::max(0,std::min(height-1,y+ky));
                    Color c=get_pixel_color(px,py);
                    float w=k[ky+1][kx+1];
                    sr+=c.r()*w; sg+=c.g()*w; sb+=c.b()*w; sa+=c.a()*w;
                }
            r->set_pixel(static_cast<gl::u32>(x), static_cast<gl::u32>(y),
                         static_cast<gl::u8>(sr),static_cast<gl::u8>(sg),
                         static_cast<gl::u8>(sb),static_cast<gl::u8>(sa));
        }
    return r;
}
    ...
    for (int y = 0; y < height; y++)
        for (int x = 0; x < width; x++)
        {
            ...
            for (int ky=-1;ky<=1;ky++)
                ...
                {
                    int px=std::max(0,std::min(width-1,x+kx)), py=std::max(0,std::min(height-1,y+ky));
                    Color c=get_pixel_color(px,py); float w=k[ky+1][kx+1];
                    sr+=c.r()*w; sg+=c.g()*w; sb+=c.b()*w;
                }
        ...
        for (int x = 0; x < width; x++)
        {
            ...
            for (int ky=-1;ky<=1;ky++)
                for (int kx=-1;kx<=1;kx++)
                {
                    ...
                    gx+=g*sx[ky+1][kx+1]; gy+=g*sy[ky+1][kx+1];
                }
...
Pixmap* Pixmap::crop(const IntRect &rect) const
{
    ...
    if (!r->pixels) { delete r; return nullptr; }
    int bpr = w * components;
    for (int row = 0; row < h; row++)
        memcpy(r->pixels + row * bpr,
               pixels + ((y + row) * width + x) * components,
...
Pixmap* Pixmap::crop_extended(const IntRect &rect, bool fill_transparent) const
{
    ...
    if (fill_transparent && (components == 4 || components == 2))
        ...
        r->fill(static_cast<gl::u8>(0), static_cast<gl::u8>(0),
    ...
    for (int row = 0; row < ch; row++)
        memcpy(r->pixels + ((doy + row) * w + dox) * components,
               pixels + ((sy0 + row) * width + sx0) * components,
               static_cast<size_t>(bpr));
    return r;
}
