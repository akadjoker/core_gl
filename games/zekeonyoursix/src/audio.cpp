...
namespace audio
{
    ...
    // Decodes an encoded clip (wav/ogg/mp3/flac — whatever miniaudio's
    // built-in decoders + stb_vorbis handle) straight from a memory buffer
    // instead of a path, so loading can go through fs::getFilesystem()'s
    // multi-root search (see readFileBytes below) rather than miniaudio's
    // own file I/O, which only ever sees the process's raw cwd. `buffer`
    // owns a copy of the encoded bytes for the decoder's whole lifetime —
    // ma_decoder_init_memory does not copy what you hand it.
    struct MemoryDecoderSource : ProceduralSource
    {
        ma_decoder decoder{};
        std::vector<unsigned char> buffer;
        bool ok = false;
        MemoryDecoderSource(const void *data, size_t sizeBytes)
        {
            const unsigned char *bytes = static_cast<const unsigned char *>(data);
            buffer.assign(bytes, bytes + sizeBytes);
            ok = ma_decoder_init_memory(buffer.data(), buffer.size(), nullptr, &decoder) == MA_SUCCESS;
        }
        ~MemoryDecoderSource()
        {
            if (ok) ma_decoder_uninit(&decoder);
        }
        ma_data_source *getDataSource() override { return ok ? &decoder : nullptr; }
    };
    ...
    // createSfx/createMusic hand the path straight to miniaudio's own file
    // I/O, which only ever looks at the process's raw cwd — no multi-root
    // search, unlike every other asset in this game (see fs::Filesystem).
    // These two decode from an in-memory buffer instead, so the caller can
    // read the bytes via fs::getFilesystem().readFile() first and it'll
    // resolve the same way models/textures/the track file already do.
    static Engine::SoundId createFromMemory(Engine::Impl *impl, bool isMusic, const void *data,
                                            size_t sizeBytes)
    {
        if (!impl || !data || sizeBytes == 0)
            return 0;

        auto *source = new MemoryDecoderSource(data, sizeBytes);
        if (!source->ok)
        {
            delete source;
            return 0;
        ...
        Engine::Impl::SoundDef *soundDef = new Engine::Impl::SoundDef();
        soundDef->id = impl->nextSoundId;
        soundDef->isMusic = isMusic;
        soundDef->procedural = source;
    ...
    Engine::SoundId Engine::createSfxFromMemory(const void *data, size_t sizeBytes)
    {
        return createFromMemory(impl, false, data, sizeBytes);
    }

    Engine::SoundId Engine::createMusicFromMemory(const void *data, size_t sizeBytes)
    {
        return createFromMemory(impl, true, data, sizeBytes);
    }
    ...
    Engine::Handle Engine::playSfxLoop(SoundId soundId, float volume, float pitch, float pan)
    {
        Impl::SoundDef *soundDef = findSound(impl, soundId, false, true);
        if (!soundDef)
            return 0;

        return createVoice(impl, soundDef, true, volume, pitch, pan);
    }
