...
namespace audio
{
    ...
    public:
        ...
        SoundId createSfxFromMemory(const void *data, size_t sizeBytes);
        SoundId createMusicFromMemory(const void *data, size_t sizeBytes);
        ...
        // like playSfx but looping and in the sfx group/volume bucket, not
        // music — playMusic() only ever tracks one active music handle
        // (calling it again stops whatever was playing), which is wrong
        // for something like a continuous engine hum that needs to
        // coexist with an actual music track.
        Handle playSfxLoop(SoundId soundId, float volume = 1.0f, float pitch = 1.0f, float pan = 0.0f);
