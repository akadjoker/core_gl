// TerrainLodNode demo: GeoMipMap terrain — one big heightmap whose patches
// drop geometric detail with distance, with T-junction stitching between
// LOD levels. The dynamic index buffer only rebuilds when the camera moves,
// and the triangle count prints so the LOD is measurable, not just visible.
//
// Controls: WASD/QE + mouse, LSHIFT fast, F10 gif, ESC quit.

#include "demo_app.hpp"
#include "demo_fly.hpp"
#include <scene/Scene.hpp>
#include <scene/SceneRenderer.hpp>
#include <scene/Material.hpp>
#include <scene/TerrainLodNode.hpp>
#include <scene/AssetManager.hpp>
#include <vector>

int main(int argc, char** argv)
{
    const int maxFrames = (argc > 1) ? atoi(argv[1]) : 0;

    DemoApp app;
    if (!app.Create("coregl - LOD terrain (GeoMipMap)")) return 1;
    printf("controls: WASD/QE + mouse | LSHIFT fast | F10 gif\n");

    SceneRenderer renderer;
    if (!renderer.init())
    {
        fprintf(stderr, "renderer init failed\n");
        app.Destroy();
        return 1;
    }
    renderer.set_light_dir(Vec3(0.45f, -0.7f, -0.35f));
    renderer.set_sky_enabled(true);

    Scene scene;
    assets::AssetManager& assets = assets::AssetManager::instance();

    TerrainLodNode* terrain = scene.root().create_child<TerrainLodNode>("lod", 4, 17);
    if (!terrain->load_heightmap("assets/terrain/terrain-heightmap.png", 0.12f, 1))
    {
        // never abort over a missing asset: procedural stand-in hills
        std::vector<float> hs((size_t)256 * 256);
        for (int z = 0; z < 256; ++z)
            for (int x = 0; x < 256; ++x)
                hs[(size_t)z * 256 + x] = 0.5f + 0.3f * sinf(x * 0.05f) * cosf(z * 0.055f);
        terrain->build(hs, 256, 0.12f);
    }
    terrain->set_texture_scale(24.f);
    terrain->set_terrain_scale(Vec3(1000.f, 400.f, 1000.f));
    Material* mat = scene.create_material();
    mat->diffuse = assets.loadTexture("ground", "assets/terrain/terrain-texture.jpg");
    terrain->set_material(mat);

    Camera3D* camera = scene.root().create_child<Camera3D>("fly");
    camera->set_perspective(55.f, 0.5f, 2000.f);
    camera->set_position(500.f, 90.f, 1080.f); // above the south edge, looking in

    scene.set_active_camera(camera);
    scene.ready();

    FlyCam fly;
    fly.speed = 60.f;
    fly.pitch = -0.22f;
    gl::u64 lastTicks = SDL_GetPerformanceCounter();
    const gl::u64 freq = SDL_GetPerformanceFrequency();

    int frame = 0;
    bool running = true;
    while (running)
    {
        SDL_Event ev;
        while (SDL_PollEvent(&ev))
        {
            if (ev.type == SDL_QUIT) running = false;
            if (ev.type == SDL_KEYDOWN)
            {
                if (ev.key.keysym.sym == SDLK_ESCAPE) running = false;
                if (ev.key.keysym.sym == SDLK_F10)
                {
                    int gw, gh;
                    app.DrawableSize(&gw, &gh);
                    app.gif.Toggle(gw, gh);
                }
            }
            fly.handle(ev);
        }
        if (!running) break;

        gl::u64 now = SDL_GetPerformanceCounter();
        float dt = (float)((double)(now - lastTicks) / (double)freq);
        lastTicks = now;
        if (dt > 0.1f) dt = 0.1f;

        fly.apply(camera, dt);

        // LOD selection follows the camera
        terrain->set_camera_position(camera->get_position());
        terrain->set_camera_forward(Mat4(camera->get_global_rotation()) * Vec3(0.f, 0.f, -1.f));

        scene.update(dt);
        int w, h;
        app.DrawableSize(&w, &h);
        renderer.render(scene, w, h);
        app.EndFrame();

        ++frame;
        if (maxFrames > 0 && frame >= maxFrames) running = false;
    }

    printf("frames: %d | items/frame: %d\n", frame, renderer.last_item_count());
    scene.release_gpu();
    assets.release();
    renderer.release();
    app.Destroy();
    return 0;
}
