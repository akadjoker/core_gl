...
int main(int argc, char** argv)
{
    ...
    // resources
    Mesh terrainMesh, cubeMesh;
    gl::Texture terrainTex;
    ...
    Material cubeMat(Vec3(0.85f, 0.35f, 0.25f));

    // the tree
    Scene scene;

    MeshInstance* terrain = (MeshInstance*)scene.root().add_child(new MeshInstance("terrain"));
    ...
    WaterNode* water = (WaterNode*)scene.root().add_child(new WaterNode("water"));
    ...
    water->release_gpu();
    terrainMesh.release_gpu();
    cubeMesh.release_gpu();
    terrainTex.Release();
    renderer.release();
    app.Destroy();
    return 0;
